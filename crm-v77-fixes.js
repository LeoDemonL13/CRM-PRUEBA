...
function bind(id,fn){const el=$(id); if(el&&!el.dataset.v77Bound){el.dataset.v77Bound='1'; el.addEventListener('click',e=>{try{fn(e)}catch(err){console.warn('[V77]',err)}})}}
function bindModalFallbacks(){
  ...
  ['config-modal','wa-config-modal','whatsapp-guide-modal','device-modal','device-token-modal'].forEach(id=>{const m=$(id); if(m&&!m.dataset.v77Backdrop){m.dataset.v77Backdrop='1'; m.addEventListener('click',e=>{if(e.target===m)hide(id)})}});
}
function hardenScrollableModals(){document.querySelectorAll('.profile-modal').forEach(m=>m.classList.add('crm-v77-compact-modal'));}
