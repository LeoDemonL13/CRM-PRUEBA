(function () {
    ...
    function sectionsForRole(){const role=currentRole();const profile=sidebarProfileKey(role);if(role==='administrador'){const base=(profile==='almacen'?warehouseSections:(profileSections[profile]||warehouseSections)).map(section=>({title:section.title,items:[...section.items]}));base.push({title:'Administración',items:[['ADM.cotizador-programas.html','Cotizador de programas','report'],['ADM.importaciones.html','Centro de importaciones','delivery'],['ADM.limpieza.html','Limpieza de pruebas','server']]});return base}return ['jefe_almacen','almacen'].includes(role)?warehouseSections:(profileSections[profile]||profileSections[role]||profileSections.consulta)}
