...
(function () {
    ...
    const SUPABASE_URL = textConfig(runtimeConfig.supabaseUrl) || 'https://pibvfosvufbwbnwfwbmw.supabase.co';
    const SUPABASE_PUBLISHABLE_KEY = textConfig(runtimeConfig.supabasePublishableKey) || 'sb_publishable_wBuo5rXuwnytiH9HLqLzBw__SdFx7NP';
    ...
    if (client?.auth && typeof client.auth.updateUser === 'function') {
        const nexusOriginalUpdateUser = client.auth.updateUser.bind(client.auth);
        client.auth.updateUser = async function(attributes, options) {
            if (attributes && Object.prototype.hasOwnProperty.call(attributes, 'password')) {
                return {
                    data: { user: null },
                    error: new Error('El cambio de contraseña está deshabilitado en Nexus Obsidian.')
                };
            }
            return nexusOriginalUpdateUser(attributes, options);
        };
    }
    ...
    async function getMyProfile() {
        ...
        return {
            ...
            tipoCuenta: text(profile?.tipo_cuenta),
            tipo_cuenta: text(profile?.tipo_cuenta),
