...
alter table public.vehiculos add column if not exists combustible_grado text;
alter table public.vehiculos add column if not exists numero_motor text;
alter table public.vehiculos add column if not exists cilindros integer;
alter table public.vehiculos add column if not exists ultima_verificacion date;
alter table public.vehiculos add column if not exists engomado text;

alter table public.vehiculos drop constraint if exists vehiculos_cilindros_v148_check;
alter table public.vehiculos add constraint vehiculos_cilindros_v148_check check(cilindros is null or cilindros between 0 and 24);

alter table public.vehiculos_viajes add column if not exists estado_base_snapshot jsonb not null default '{}'::jsonb;
alter table public.vehiculos_viajes add column if not exists danos_salida jsonb not null default '[]'::jsonb;
alter table public.vehiculos_viajes add column if not exists danos_entrada jsonb not null default '[]'::jsonb;
alter table public.vehiculos_viajes add column if not exists combustible_salida numeric;
alter table public.vehiculos_viajes add column if not exists combustible_regreso numeric;
alter table public.vehiculos_viajes add column if not exists vida_llantas_salida numeric;
alter table public.vehiculos_viajes add column if not exists vida_llantas_regreso numeric;

alter table public.vehiculos_viajes drop constraint if exists vehiculos_viajes_combustible_salida_v148_check;
alter table public.vehiculos_viajes add constraint vehiculos_viajes_combustible_salida_v148_check check(combustible_salida is null or combustible_salida between 0 and 1);
alter table public.vehiculos_viajes drop constraint if exists vehiculos_viajes_combustible_regreso_v148_check;
alter table public.vehiculos_viajes add constraint vehiculos_viajes_combustible_regreso_v148_check check(combustible_regreso is null or combustible_regreso between 0 and 1);
alter table public.vehiculos_viajes drop constraint if exists vehiculos_viajes_vida_llantas_salida_v148_check;
alter table public.vehiculos_viajes add constraint vehiculos_viajes_vida_llantas_salida_v148_check check(vida_llantas_salida is null or vida_llantas_salida between 0 and 100);
alter table public.vehiculos_viajes drop constraint if exists vehiculos_viajes_vida_llantas_regreso_v148_check;
alter table public.vehiculos_viajes add constraint vehiculos_viajes_vida_llantas_regreso_v148_check check(vida_llantas_regreso is null or vida_llantas_regreso between 0 and 100);

create table if not exists public.vehiculos_estado_base(
    id bigint generated by default as identity primary key,
    vehiculo_id bigint not null unique references public.vehiculos(id) on update cascade on delete cascade,
    combustible_tipo text,
    combustible_grado text,
    combustible_nivel_referencia numeric,
    vida_llantas numeric,
    presion_del_izq numeric,
    presion_del_der numeric,
    presion_tras_izq numeric,
    presion_tras_der numeric,
    kilometraje_referencia numeric,
...
alter table public.vehiculos_estado_base add column if not exists combustible_tipo text;
alter table public.vehiculos_estado_base add column if not exists combustible_grado text;
alter table public.vehiculos_estado_base add column if not exists combustible_nivel_referencia numeric;
alter table public.vehiculos_estado_base add column if not exists vida_llantas numeric;
alter table public.vehiculos_estado_base add column if not exists presion_del_izq numeric;
alter table public.vehiculos_estado_base add column if not exists presion_del_der numeric;
alter table public.vehiculos_estado_base add column if not exists presion_tras_izq numeric;
alter table public.vehiculos_estado_base add column if not exists presion_tras_der numeric;
alter table public.vehiculos_estado_base add column if not exists kilometraje_referencia numeric;
alter table public.vehiculos_estado_base add column if not exists ultima_verificacion date;
alter table public.vehiculos_estado_base add column if not exists proximo_servicio_fecha date;
alter table public.vehiculos_estado_base add column if not exists proximo_servicio_km numeric;
alter table public.vehiculos_estado_base add column if not exists responsable text;
alter table public.vehiculos_estado_base add column if not exists condicion_general text not null default 'buena';
alter table public.vehiculos_estado_base add column if not exists danos jsonb not null default '[]'::jsonb;
alter table public.vehiculos_estado_base add column if not exists observaciones text;
alter table public.vehiculos_estado_base add column if not exists actualizado_por uuid references auth.users(id) on delete set null;
alter table public.vehiculos_estado_base add column if not exists actualizado_por_nombre text;
alter table public.vehiculos_estado_base add column if not exists created_at timestamptz not null default now();
alter table public.vehiculos_estado_base add column if not exists updated_at timestamptz not null default now();

alter table public.vehiculos_estado_base drop constraint if exists vehiculos_estado_base_combustible_v148_check;
alter table public.vehiculos_estado_base add constraint vehiculos_estado_base_combustible_v148_check check(combustible_nivel_referencia is null or combustible_nivel_referencia between 0 and 1);
alter table public.vehiculos_estado_base drop constraint if exists vehiculos_estado_base_llantas_v148_check;
alter table public.vehiculos_estado_base add constraint vehiculos_estado_base_llantas_v148_check check(vida_llantas is null or vida_llantas between 0 and 100);
alter table public.vehiculos_estado_base drop constraint if exists vehiculos_estado_base_presiones_v148_check;
alter table public.vehiculos_estado_base add constraint vehiculos_estado_base_presiones_v148_check check(
    (presion_del_izq is null or presion_del_izq between 0 and 100) and
    (presion_del_der is null or presion_del_der between 0 and 100) and
    (presion_tras_izq is null or presion_tras_izq between 0 and 100) and
    (presion_tras_der is null or presion_tras_der between 0 and 100)
);
alter table public.vehiculos_estado_base drop constraint if exists vehiculos_estado_base_condicion_v148_check;
alter table public.vehiculos_estado_base add constraint vehiculos_estado_base_condicion_v148_check check(condicion_general in ('excelente','buena','regular','mala','fuera_servicio'));
alter table public.vehiculos_estado_base drop constraint if exists vehiculos_estado_base_danos_array_v148_check;
alter table public.vehiculos_estado_base add constraint vehiculos_estado_base_danos_array_v148_check check(jsonb_typeof(danos)='array');

create table if not exists public.vehiculos_estado_base_historial(
    id bigint generated by default as identity primary key,
    vehiculo_id bigint not null references public.vehiculos(id) on update cascade on delete cascade,
...
create index if not exists vehiculos_estado_base_historial_vehiculo_idx on public.vehiculos_estado_base_historial(vehiculo_id,created_at desc);

alter table public.vehiculos_estado_base enable row level security;
alter table public.vehiculos_estado_base_historial enable row level security;

drop policy if exists vehiculos_estado_base_lectura_v148 on public.vehiculos_estado_base;
create policy vehiculos_estado_base_lectura_v148 on public.vehiculos_estado_base for select to authenticated using(
    public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen','gerente_general','subgerente','logistica','consulta','sky_demo'])
);
drop policy if exists vehiculos_estado_base_escritura_v148 on public.vehiculos_estado_base;
create policy vehiculos_estado_base_escritura_v148 on public.vehiculos_estado_base for all to authenticated using(
    public.crm_usuario_tiene_rol(array['administrador','jefe_almacen'])
) with check(
    public.crm_usuario_tiene_rol(array['administrador','jefe_almacen'])
);

drop policy if exists vehiculos_estado_base_historial_lectura_v148 on public.vehiculos_estado_base_historial;
create policy vehiculos_estado_base_historial_lectura_v148 on public.vehiculos_estado_base_historial for select to authenticated using(
    public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen','gerente_general','subgerente','logistica','consulta','sky_demo'])
);
drop policy if exists vehiculos_estado_base_historial_escritura_v148 on public.vehiculos_estado_base_historial;
create policy vehiculos_estado_base_historial_escritura_v148 on public.vehiculos_estado_base_historial for insert to authenticated with check(
    public.crm_usuario_tiene_rol(array['administrador','jefe_almacen'])
);

grant select,insert,update,delete on public.vehiculos_estado_base to authenticated;
grant select,insert on public.vehiculos_estado_base_historial to authenticated;
grant usage,select on sequence public.vehiculos_estado_base_id_seq to authenticated;
grant usage,select on sequence public.vehiculos_estado_base_historial_id_seq to authenticated;

create or replace function public.crm_guardar_estado_base_vehiculo_v148(
    p_vehiculo_id bigint,
    p_datos jsonb default '{}'::jsonb
)
returns public.vehiculos_estado_base
language plpgsql
security definer
set search_path=public
as $$
declare
    v_result public.vehiculos_estado_base%rowtype;
    v_damage jsonb;
    v_damage_count integer:=0;
    v_condition text:=lower(btrim(coalesce(p_datos->>'condicion_general','buena')));
    v_user_name text;
    v_fuel numeric;
    v_tire numeric;
    v_psi_fl numeric;
...
begin
    ...
    if not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen']) then
        raise exception using errcode='42501',message='Solo Jefe de Almacén o Administrador puede actualizar el estado base de vehículos.';
    end if;
    if coalesce(p_vehiculo_id,0)<=0 or not exists(select 1 from public.vehiculos where id=p_vehiculo_id) then raise exception 'El vehículo no existe.'; end if;
    if v_condition not in ('excelente','buena','regular','mala','fuera_servicio') then raise exception 'La condición general no es válida.'; end if;
    if coalesce(jsonb_typeof(p_datos->'danos'),'array')<>'array' then raise exception 'La lista de daños no es válida.'; end if;
    v_damage_count:=jsonb_array_length(coalesce(p_datos->'danos','[]'::jsonb));
    if v_damage_count>200 then raise exception 'El estado base no puede contener más de 200 marcas de daño.'; end if;
    for v_damage in select value from jsonb_array_elements(coalesce(p_datos->'danos','[]'::jsonb)) loop
        if coalesce(v_damage->>'vista','') not in ('superior','frontal','lado_izquierdo','lado_derecho','trasera') then raise exception 'Una marca contiene una vista no válida.'; end if;
        if coalesce(v_damage->>'tipo','') not in ('golpe','rayon','abolladura','faro','defensa','puerta','espejo','otro') then raise exception 'Una marca contiene un tipo de daño no válido.'; end if;
        if coalesce(nullif(v_damage->>'x','')::numeric,-1)<0 or coalesce(nullif(v_damage->>'x','')::numeric,101)>100 or coalesce(nullif(v_damage->>'y','')::numeric,-1)<0 or coalesce(nullif(v_damage->>'y','')::numeric,101)>100 then raise exception 'Una marca de daño está fuera del área permitida.'; end if;
    end loop;
    v_fuel:=nullif(p_datos->>'combustible_nivel_referencia','')::numeric;
    v_tire:=nullif(p_datos->>'vida_llantas','')::numeric;
    v_psi_fl:=nullif(p_datos->>'presion_del_izq','')::numeric;
    v_psi_fr:=nullif(p_datos->>'presion_del_der','')::numeric;
    v_psi_rl:=nullif(p_datos->>'presion_tras_izq','')::numeric;
    v_psi_rr:=nullif(p_datos->>'presion_tras_der','')::numeric;
    if v_fuel is not null and (v_fuel<0 or v_fuel>1) then raise exception 'El nivel de combustible debe estar entre vacío y lleno.'; end if;
    if v_tire is not null and (v_tire<0 or v_tire>100) then raise exception 'La vida de llantas debe estar entre 0 y 100 por ciento.'; end if;
    if (v_psi_fl is not null and (v_psi_fl<0 or v_psi_fl>100)) or (v_psi_fr is not null and (v_psi_fr<0 or v_psi_fr>100)) or (v_psi_rl is not null and (v_psi_rl<0 or v_psi_rl>100)) or (v_psi_rr is not null and (v_psi_rr<0 or v_psi_rr>100)) then raise exception 'La presión de llantas debe estar entre 0 y 100 PSI.'; end if;

    select coalesce(nullif(btrim(p.nombre),''),u.email,'Usuario') into v_user_name
    from public.perfiles_usuario p left join auth.users u on u.id=p.id where p.id=auth.uid() and p.activo=true;
    v_user_name:=coalesce(v_user_name,'Usuario');

    insert into public.vehiculos_estado_base(
        vehiculo_id,combustible_tipo,combustible_grado,combustible_nivel_referencia,vida_llantas,
        presion_del_izq,presion_del_der,presion_tras_izq,presion_tras_der,kilometraje_referencia,
        ultima_verificacion,proximo_servicio_fecha,proximo_servicio_km,responsable,condicion_general,
        danos,observaciones,actualizado_por,actualizado_por_nombre,created_at,updated_at
    ) values(
        p_vehiculo_id,nullif(lower(btrim(coalesce(p_datos->>'combustible_tipo',''))),''),nullif(lower(btrim(coalesce(p_datos->>'combustible_grado',''))),''),v_fuel,v_tire,
        v_psi_fl,v_psi_fr,v_psi_rl,v_psi_rr,nullif(p_datos->>'kilometraje_referencia','')::numeric,
        nullif(p_datos->>'ultima_verificacion','')::date,nullif(p_datos->>'proximo_servicio_fecha','')::date,nullif(p_datos->>'proximo_servicio_km','')::numeric,nullif(btrim(coalesce(p_datos->>'responsable','')),''),v_condition,
        coalesce(p_datos->'danos','[]'::jsonb),nullif(btrim(coalesce(p_datos->>'observaciones','')),''),auth.uid(),v_user_name,now(),now()
    )
    on conflict(vehiculo_id) do update set
        combustible_tipo=excluded.combustible_tipo,combustible_grado=excluded.combustible_grado,combustible_nivel_referencia=excluded.combustible_nivel_referencia,vida_llantas=excluded.vida_llantas,
        presion_del_izq=excluded.presion_del_izq,presion_del_der=excluded.presion_del_der,presion_tras_izq=excluded.presion_tras_izq,presion_tras_der=excluded.presion_tras_der,kilometraje_referencia=excluded.kilometraje_referencia,
        ultima_verificacion=excluded.ultima_verificacion,proximo_servicio_fecha=excluded.proximo_servicio_fecha,proximo_servicio_km=excluded.proximo_servicio_km,responsable=excluded.responsable,condicion_general=excluded.condicion_general,
        danos=excluded.danos,observaciones=excluded.observaciones,actualizado_por=excluded.actualizado_por,actualizado_por_nombre=excluded.actualizado_por_nombre,updated_at=now()
    returning * into v_result;

    update public.vehiculos set
        combustible=coalesce(v_result.combustible_tipo,combustible),
        combustible_grado=coalesce(v_result.combustible_grado,combustible_grado),
        kilometraje=greatest(coalesce(kilometraje,0),coalesce(v_result.kilometraje_referencia,0)),
        ultima_verificacion=coalesce(v_result.ultima_verificacion,ultima_verificacion),
        responsable=coalesce(v_result.responsable,responsable),
        updated_at=now()
    where id=p_vehiculo_id;

    insert into public.vehiculos_estado_base_historial(vehiculo_id,snapshot,actualizado_por,actualizado_por_nombre,created_at)
    values(p_vehiculo_id,to_jsonb(v_result)-'id',auth.uid(),v_user_name,now());
    return v_result;
end;
$$;
revoke all on function public.crm_guardar_estado_base_vehiculo_v148(bigint,jsonb) from public,anon;
grant execute on function public.crm_guardar_estado_base_vehiculo_v148(bigint,jsonb) to authenticated,service_role;
...
declare
    ...
    v_aviso_seguro integer:=greatest(1,least(180,coalesce(nullif(p_datos->>'aviso_seguro_dias','')::integer,30)));
begin
    if auth.uid() is not null and not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen','rh']) then raise exception using errcode='42501',message='Tu perfil no tiene permiso para guardar o editar vehículos.'; end if;
    if v_numero='' then raise exception 'El nombre del vehículo es obligatorio.'; end if;
    if v_marca='' or v_modelo='' then raise exception 'Marca y modelo son obligatorios.'; end if;
    if v_base_tipo not in ('','almacen','proyecto','sede','oficina','taller','patio','otra') then raise exception 'El tipo de ubicación base no es válido.'; end if;
    if nullif(p_datos->>'cilindros','')::integer is not null and nullif(p_datos->>'cilindros','')::integer not between 0 and 24 then raise exception 'El número de cilindros no es válido.'; end if;

    if coalesce(p_id,0)=0 then
        insert into public.vehiculos(
            numero_economico,apodo,placas,vin,marca,modelo,anio,tipo,color,numero_motor,cilindros,combustible,combustible_grado,transmision,
            capacidad_carga,capacidad_personas,distribucion_asientos,kilometraje,propiedad,estado,
            almacen_base_id,ubicacion_base_tipo,ubicacion_base_referencia,ubicacion_base_nombre,proyecto,asignado_a,responsable,
            aseguradora,poliza_seguro,vigencia_seguro,aviso_seguro_dias,tarjeta_circulacion,vigencia_tarjeta,
            ultima_verificacion,proxima_verificacion,engomado,fecha_adquisicion,costo_adquisicion,imagen_url,notas,activo,created_at,updated_at
        ) values(
            v_numero,nullif(btrim(coalesce(p_datos->>'apodo','')),''),nullif(upper(btrim(coalesce(p_datos->>'placas',''))),''),nullif(upper(btrim(coalesce(p_datos->>'vin',''))),''),
            v_marca,v_modelo,nullif(p_datos->>'anio','')::integer,coalesce(nullif(p_datos->>'tipo',''),'pickup'),nullif(p_datos->>'color',''),nullif(upper(btrim(coalesce(p_datos->>'numero_motor',''))),''),nullif(p_datos->>'cilindros','')::integer,nullif(p_datos->>'combustible',''),nullif(p_datos->>'combustible_grado',''),nullif(p_datos->>'transmision',''),
            greatest(0,coalesce(nullif(p_datos->>'capacidad_carga','')::numeric,0)),greatest(0,coalesce(nullif(p_datos->>'capacidad_personas','')::integer,0)),coalesce(p_datos->'distribucion_asientos','{"version":1,"filas":[]}'::jsonb),
            greatest(0,coalesce(nullif(p_datos->>'kilometraje','')::numeric,0)),coalesce(nullif(p_datos->>'propiedad',''),'empresa'),coalesce(nullif(p_datos->>'estado',''),'disponible'),
            nullif(p_datos->>'almacen_base_id','')::bigint,nullif(v_base_tipo,''),nullif(btrim(coalesce(p_datos->>'ubicacion_base_referencia','')),''),nullif(btrim(coalesce(p_datos->>'ubicacion_base_nombre','')),''),
            nullif(p_datos->>'proyecto',''),nullif(p_datos->>'asignado_a',''),nullif(p_datos->>'responsable',''),nullif(p_datos->>'aseguradora',''),nullif(p_datos->>'poliza_seguro',''),nullif(p_datos->>'vigencia_seguro','')::date,v_aviso_seguro,
            nullif(p_datos->>'tarjeta_circulacion',''),nullif(p_datos->>'vigencia_tarjeta','')::date,nullif(p_datos->>'ultima_verificacion','')::date,nullif(p_datos->>'proxima_verificacion','')::date,nullif(p_datos->>'engomado',''),nullif(p_datos->>'fecha_adquisicion','')::date,
            greatest(0,coalesce(nullif(p_datos->>'costo_adquisicion','')::numeric,0)),nullif(p_datos->>'imagen_url',''),nullif(p_datos->>'notas',''),coalesce(nullif(p_datos->>'activo','')::boolean,true),now(),now()
        ) returning * into v_result;
    else
        update public.vehiculos set
            numero_economico=v_numero,apodo=nullif(btrim(coalesce(p_datos->>'apodo','')),''),placas=nullif(upper(btrim(coalesce(p_datos->>'placas',''))),''),vin=nullif(upper(btrim(coalesce(p_datos->>'vin',''))),''),
            marca=v_marca,modelo=v_modelo,anio=nullif(p_datos->>'anio','')::integer,tipo=coalesce(nullif(p_datos->>'tipo',''),'pickup'),color=nullif(p_datos->>'color',''),numero_motor=nullif(upper(btrim(coalesce(p_datos->>'numero_motor',''))),''),cilindros=nullif(p_datos->>'cilindros','')::integer,combustible=nullif(p_datos->>'combustible',''),combustible_grado=nullif(p_datos->>'combustible_grado',''),transmision=nullif(p_datos->>'transmision',''),
            capacidad_carga=greatest(0,coalesce(nullif(p_datos->>'capacidad_carga','')::numeric,0)),capacidad_personas=greatest(0,coalesce(nullif(p_datos->>'capacidad_personas','')::integer,0)),distribucion_asientos=coalesce(p_datos->'distribucion_asientos','{"version":1,"filas":[]}'::jsonb),
            kilometraje=greatest(0,coalesce(nullif(p_datos->>'kilometraje','')::numeric,0)),propiedad=coalesce(nullif(p_datos->>'propiedad',''),'empresa'),estado=coalesce(nullif(p_datos->>'estado',''),'disponible'),
            almacen_base_id=nullif(p_datos->>'almacen_base_id','')::bigint,ubicacion_base_tipo=nullif(v_base_tipo,''),ubicacion_base_referencia=nullif(btrim(coalesce(p_datos->>'ubicacion_base_referencia','')),''),ubicacion_base_nombre=nullif(btrim(coalesce(p_datos->>'ubicacion_base_nombre','')),''),
            proyecto=nullif(p_datos->>'proyecto',''),asignado_a=nullif(p_datos->>'asignado_a',''),responsable=nullif(p_datos->>'responsable',''),aseguradora=nullif(p_datos->>'aseguradora',''),poliza_seguro=nullif(p_datos->>'poliza_seguro',''),
            vigencia_seguro=nullif(p_datos->>'vigencia_seguro','')::date,aviso_seguro_dias=v_aviso_seguro,tarjeta_circulacion=nullif(p_datos->>'tarjeta_circulacion',''),vigencia_tarjeta=nullif(p_datos->>'vigencia_tarjeta','')::date,
            ultima_verificacion=nullif(p_datos->>'ultima_verificacion','')::date,proxima_verificacion=nullif(p_datos->>'proxima_verificacion','')::date,engomado=nullif(p_datos->>'engomado',''),fecha_adquisicion=nullif(p_datos->>'fecha_adquisicion','')::date,costo_adquisicion=greatest(0,coalesce(nullif(p_datos->>'costo_adquisicion','')::numeric,0)),
            imagen_url=nullif(p_datos->>'imagen_url',''),notas=nullif(p_datos->>'notas',''),activo=coalesce(nullif(p_datos->>'activo','')::boolean,activo),updated_at=now()
        where id=p_id returning * into v_result;
        if not found then raise exception 'No existe el vehículo con ID %.',p_id; end if;
    end if;
    return v_result;
exception when unique_violation then raise exception using errcode='23505',message='El nombre del vehículo, las placas o el VIN ya están registrados.';
end;
$$;
revoke all on function public.crm_guardar_vehiculo(bigint,jsonb) from public,anon;
grant execute on function public.crm_guardar_vehiculo(bigint,jsonb) to authenticated,service_role;
...
alter table public.vehiculos_viajes add column if not exists folio text;
alter table public.vehiculos_viajes add column if not exists ciudad_estado text;
alter table public.vehiculos_viajes add column if not exists conductor_personal_id bigint references public.rh_personal(id) on update cascade on delete set null;
alter table public.vehiculos_viajes add column if not exists checklist_salida jsonb not null default '{}'::jsonb;
alter table public.vehiculos_viajes add column if not exists checklist_entrada jsonb not null default '{}'::jsonb;
alter table public.vehiculos_viajes add column if not exists elaboro_firma text;
alter table public.vehiculos_viajes add column if not exists elaboro_firmado_at timestamptz;
alter table public.vehiculos_viajes add column if not exists autorizo_user_id uuid references auth.users(id) on delete set null;
alter table public.vehiculos_viajes add column if not exists autorizo_nombre text;
alter table public.vehiculos_viajes add column if not exists autorizo_firma text;
alter table public.vehiculos_viajes add column if not exists autorizo_firma_slot integer;
alter table public.vehiculos_viajes add column if not exists autorizo_firmado_at timestamptz;
alter table public.vehiculos_viajes add column if not exists responsable_modo text;
alter table public.vehiculos_viajes add column if not exists responsable_personal_id bigint references public.rh_personal(id) on update cascade on delete set null;
alter table public.vehiculos_viajes add column if not exists responsable_nombre text;
alter table public.vehiculos_viajes add column if not exists responsable_empresa text;
alter table public.vehiculos_viajes add column if not exists responsable_firma text;
alter table public.vehiculos_viajes add column if not exists responsable_firmado_at timestamptz;
alter table public.vehiculos_viajes add column if not exists politicas_aceptadas boolean not null default false;

create unique index if not exists vehiculos_viajes_folio_v148_uidx on public.vehiculos_viajes(folio) where folio is not null;

create table if not exists public.vehiculos_conductores_autorizados(
    id bigint generated by default as identity primary key,
    vehiculo_id bigint not null references public.vehiculos(id) on update cascade on delete cascade,
    personal_id bigint not null references public.rh_personal(id) on update cascade on delete cascade,
    activo boolean not null default true,
    ...
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now(),
    unique(vehiculo_id,personal_id)
);
create index if not exists vehiculos_conductores_vehiculo_v148_idx on public.vehiculos_conductores_autorizados(vehiculo_id,activo);
alter table public.vehiculos_conductores_autorizados enable row level security;
drop policy if exists vehiculos_conductores_lectura_v148 on public.vehiculos_conductores_autorizados;
create policy vehiculos_conductores_lectura_v148 on public.vehiculos_conductores_autorizados for select to authenticated using(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen','gerente_general','subgerente','logistica','consulta']));
drop policy if exists vehiculos_conductores_escritura_v148 on public.vehiculos_conductores_autorizados;
create policy vehiculos_conductores_escritura_v148 on public.vehiculos_conductores_autorizados for all to authenticated using(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen'])) with check(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen']));
grant select,insert,update,delete on public.vehiculos_conductores_autorizados to authenticated;
grant usage,select on sequence public.vehiculos_conductores_autorizados_id_seq to authenticated;

create or replace function public.crm_conductores_vehiculo_v148(p_vehiculo_id bigint)
returns table(personal_id bigint,numero_empleado text,nombre_completo text,puesto text,departamento text,autorizado boolean)
language plpgsql stable security definer set search_path=public
as $$
begin
    if auth.uid() is null or not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen','gerente_general','subgerente','logistica']) then raise exception using errcode='42501',message='No tienes permiso para consultar conductores.'; end if;
    return query
    select p.id,p.numero_empleado,btrim(concat_ws(' ',p.nombre,p.apellidos)),p.puesto,p.departamento,
           exists(select 1 from public.vehiculos_conductores_autorizados a where a.vehiculo_id=p_vehiculo_id and a.personal_id=p.id and a.activo=true)
    from public.rh_personal p
    where lower(coalesce(p.estado,'activo'))='activo'
    order by p.nombre,p.apellidos,p.numero_empleado;
end;
$$;
revoke all on function public.crm_conductores_vehiculo_v148(bigint) from public,anon;
grant execute on function public.crm_conductores_vehiculo_v148(bigint) to authenticated,service_role;

create or replace function public.crm_guardar_conductores_vehiculo_v148(p_vehiculo_id bigint,p_personal_ids bigint[] default '{}'::bigint[])
returns integer
language plpgsql security definer set search_path=public
as $$
declare v_count integer;
begin
    if auth.uid() is null or not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen']) then raise exception using errcode='42501',message='Solo Jefe de Almacén o Administrador puede autorizar conductores.'; end if;
    if not exists(select 1 from public.vehiculos where id=p_vehiculo_id) then raise exception 'El vehículo no existe.'; end if;
    update public.vehiculos_conductores_autorizados set activo=false,updated_at=now(),autorizado_por=auth.uid() where vehiculo_id=p_vehiculo_id;
    insert into public.vehiculos_conductores_autorizados(vehiculo_id,personal_id,activo,autorizado_por,created_at,updated_at)
    select p_vehiculo_id,p.id,true,auth.uid(),now(),now() from public.rh_personal p where p.id=any(coalesce(p_personal_ids,'{}'::bigint[])) and lower(coalesce(p.estado,'activo'))='activo'
    on conflict(vehiculo_id,personal_id) do update set activo=true,autorizado_por=excluded.autorizado_por,updated_at=now();
    select count(*) into v_count from public.vehiculos_conductores_autorizados where vehiculo_id=p_vehiculo_id and activo=true;
    return v_count;
end;
$$;
revoke all on function public.crm_guardar_conductores_vehiculo_v148(bigint,bigint[]) from public,anon;
grant execute on function public.crm_guardar_conductores_vehiculo_v148(bigint,bigint[]) to authenticated,service_role;

-- Endurece la escritura de salidas: la lectura sigue disponible para perfiles autorizados, la escritura directa queda limitada.
drop policy if exists vehiculos_viajes_lectura on public.vehiculos_viajes;
drop policy if exists vehiculos_viajes_escritura on public.vehiculos_viajes;
create policy vehiculos_viajes_lectura on public.vehiculos_viajes for select to authenticated using(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen','gerente_general','subgerente','logistica','consulta']));
create policy vehiculos_viajes_escritura on public.vehiculos_viajes for all to authenticated using(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen'])) with check(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen']));

create or replace function public.crm_validar_checklist_vehiculo_v148(p_checklist jsonb)
returns boolean
...
declare
    v_key text;
    v_value text;
    v_required text[]:=array['limpieza_exterior','limpieza_interior','vestiduras','condicion_tablero','testigo_encendido','pintura','espejos','cristales','luz_interna','luces','cuartos','stop','direccionales','plumas_parabrisas','vida_llanta','llanta_refaccion','gato_hidraulico','herramientas','cables_corriente','nivel_aceite','nivel_anticongelante','nivel_parabrisas','nivel_frenos','goteos_inferiores','manual','poliza_seguro','tarjeta_circulacion','verificacion'];
begin
    if coalesce(jsonb_typeof(p_checklist),'')<>'object' then return false; end if;
    foreach v_key in array v_required loop
        v_value:=upper(btrim(coalesce(p_checklist->>v_key,'')));
        if v_value not in ('P','X','NA') then return false; end if;
    end loop;
    return true;
end;
$$;
revoke all on function public.crm_validar_checklist_vehiculo_v148(jsonb) from public,anon;
grant execute on function public.crm_validar_checklist_vehiculo_v148(jsonb) to authenticated,service_role;

create or replace function public.crm_guardar_salida_vehiculo_v148(p_datos jsonb default '{}'::jsonb)
returns public.vehiculos_viajes
language plpgsql security definer set search_path=public
as $$
declare
    v_row public.vehiculos_viajes%rowtype;
    v_vehicle public.vehiculos%rowtype;
    v_profile public.perfiles_usuario%rowtype;
    v_driver public.rh_personal%rowtype;
    v_responsible public.rh_personal%rowtype;
    v_vehicle_id bigint:=nullif(p_datos->>'vehiculo_id','')::bigint;
    v_driver_id bigint:=nullif(p_datos->>'conductor_personal_id','')::bigint;
    v_responsible_id bigint:=nullif(p_datos->>'responsable_personal_id','')::bigint;
    v_mode text:=lower(btrim(coalesce(p_datos->>'responsable_modo','mismo_elaboro')));
    v_authorizer_sig text;
    v_authorizer_slot integer:=nullif(p_datos->>'autorizo_firma_slot','')::integer;
    v_authorizer_name text;
    v_driver_name text;
    v_responsible_name text;
    v_responsible_sig text:=nullif(p_datos->>'responsable_firma','');
    v_driver_sig text:=nullif(p_datos->>'elaboro_firma','');
    v_manual_auth_sig text:=nullif(p_datos->>'autorizo_firma_manual','');
    v_base jsonb:='{}'::jsonb;
    v_passenger jsonb;
    v_departure timestamptz:=coalesce(nullif(p_datos->>'fecha_salida','')::timestamptz,now());
    v_expected timestamptz:=nullif(p_datos->>'fecha_regreso_estimada','')::timestamptz;
begin
    if auth.uid() is null then raise exception using errcode='42501',message='Debes iniciar sesión.'; end if;
    if not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen']) then raise exception using errcode='42501',message='La salida debe ser autorizada por el Jefe de Almacén.'; end if;
    if not public.crm_validar_checklist_vehiculo_v148(coalesce(p_datos->'checklist_salida','{}'::jsonb)) then raise exception 'Completa correctamente los 28 puntos del checklist de salida.'; end if;
    select * into v_profile from public.perfiles_usuario where id=auth.uid() and activo=true;
    if not found then raise exception using errcode='42501',message='Tu perfil no está activo.'; end if;
    select * into v_vehicle from public.vehiculos where id=v_vehicle_id and activo=true;
    if not found then raise exception 'El vehículo no existe o está inactivo.'; end if;
    if coalesce(v_vehicle.estado,'disponible')<>'disponible' then raise exception 'El vehículo no está disponible para una nueva salida.'; end if;
    if exists(select 1 from public.vehiculos_viajes where vehiculo_id=v_vehicle_id and estado='en_curso') then raise exception 'El vehículo ya tiene una salida activa.'; end if;
    if v_expected is not null and v_expected<v_departure then raise exception 'El regreso estimado no puede ser anterior a la salida.'; end if;
    select * into v_driver from public.rh_personal where id=v_driver_id and lower(coalesce(estado,'activo'))='activo';
    if not found or not exists(select 1 from public.vehiculos_conductores_autorizados a where a.vehiculo_id=v_vehicle_id and a.personal_id=v_driver_id and a.activo=true) then raise exception 'El conductor no está autorizado para este vehículo.'; end if;
    v_driver_name:=btrim(concat_ws(' ',v_driver.nombre,v_driver.apellidos));
    if v_driver_sig is null or v_driver_sig !~ '^data:image/(png|jpeg|jpg|webp);base64,' or length(v_driver_sig)<100 then raise exception 'El conductor autorizado debe firmar Elaboró antes de retirar la unidad.'; end if;
    v_authorizer_name:=coalesce(nullif(btrim(v_profile.nombre),''),(select email from auth.users where id=auth.uid()),'Jefe de Almacén');
    if v_authorizer_slot between 1 and 3 then
        if v_authorizer_slot=1 then v_authorizer_sig:=nullif(btrim(coalesce(v_profile.firma_data_url,'')),'');
        elsif v_authorizer_slot=2 then v_authorizer_sig:=nullif(btrim(coalesce(v_profile.firma_2_data_url,'')),'');
        else v_authorizer_sig:=nullif(btrim(coalesce(v_profile.firma_3_data_url,'')),''); end if;
    else v_authorizer_sig:=v_manual_auth_sig; v_authorizer_slot:=null; end if;
    if v_authorizer_sig is null or v_authorizer_sig !~ '^data:image/(png|jpeg|jpg|webp);base64,' or length(v_authorizer_sig)<100 then raise exception 'Autorizó requiere una firma válida del Jefe de Almacén.'; end if;
    if v_mode='mismo_elaboro' then v_responsible_id:=v_driver_id;v_responsible_name:=v_driver_name;v_responsible_sig:=v_driver_sig;
    elsif v_mode='conductor_autorizado' then
        select * into v_responsible from public.rh_personal where id=v_responsible_id and lower(coalesce(estado,'activo'))='activo';
        if not found or not exists(select 1 from public.vehiculos_conductores_autorizados a where a.vehiculo_id=v_vehicle_id and a.personal_id=v_responsible_id and a.activo=true) then raise exception 'El Responsable asignado seleccionado no es conductor autorizado.'; end if;
        v_responsible_name:=btrim(concat_ws(' ',v_responsible.nombre,v_responsible.apellidos));
    elsif v_mode='planta' then v_responsible_name:=nullif(btrim(coalesce(p_datos->>'responsable_nombre','')),'');if v_responsible_name is null then raise exception 'Captura el nombre del Responsable asignado en planta.'; end if;
    else raise exception 'El modo de Responsable asignado no es válido.'; end if;
    select to_jsonb(b) into v_base from public.vehiculos_estado_base b where b.vehiculo_id=v_vehicle_id;
    v_base:=coalesce(v_base,'{}'::jsonb);

    insert into public.vehiculos_viajes(
        vehiculo_id,fecha_salida,fecha_regreso_estimada,conductor,conductor_personal_id,proyecto,destino,ciudad_estado,motivo,kilometraje_salida,
        combustible_salida,vida_llantas_salida,estado_base_snapshot,checklist_salida,danos_salida,observaciones,estado,
        elaboro_firma,elaboro_firmado_at,autorizo_user_id,autorizo_nombre,autorizo_firma,autorizo_firma_slot,autorizo_firmado_at,
        responsable_modo,responsable_personal_id,responsable_nombre,responsable_empresa,responsable_firma,responsable_firmado_at,politicas_aceptadas,created_at,updated_at
    ) values(
        v_vehicle_id,v_departure,v_expected,v_driver_name,v_driver_id,nullif(p_datos->>'proyecto',''),btrim(coalesce(p_datos->>'destino','')),nullif(btrim(coalesce(p_datos->>'ciudad_estado','')),''),nullif(btrim(coalesce(p_datos->>'motivo','')),''),greatest(0,coalesce(nullif(p_datos->>'kilometraje_salida','')::numeric,v_vehicle.kilometraje,0)),
        coalesce(nullif(p_datos->>'combustible_salida','')::numeric,nullif(v_base->>'combustible_nivel_referencia','')::numeric),coalesce(nullif(p_datos->>'vida_llantas_salida','')::numeric,nullif(v_base->>'vida_llantas','')::numeric),v_base,coalesce(p_datos->'checklist_salida','{}'::jsonb),coalesce(p_datos->'danos_salida',coalesce(v_base->'danos','[]'::jsonb)),nullif(btrim(coalesce(p_datos->>'observaciones','')),''),'en_curso',
        v_driver_sig,now(),auth.uid(),v_authorizer_name,v_authorizer_sig,v_authorizer_slot,now(),v_mode,v_responsible_id,v_responsible_name,nullif(btrim(coalesce(p_datos->>'responsable_empresa','')),''),v_responsible_sig,case when v_responsible_sig is null then null else now() end,coalesce(nullif(p_datos->>'politicas_aceptadas','')::boolean,false),now(),now()
    ) returning * into v_row;
    update public.vehiculos_viajes set folio='SAL-'||to_char(v_departure,'YYYY')||'-'||lpad(v_row.id::text,6,'0') where id=v_row.id returning * into v_row;
    for v_passenger in select value from jsonb_array_elements(coalesce(p_datos->'pasajeros','[]'::jsonb)) loop
        if nullif(btrim(coalesce(v_passenger->>'nombre','')),'') is not null then
            insert into public.vehiculos_viaje_pasajeros(viaje_id,nombre,puesto,contacto,asiento) values(v_row.id,btrim(v_passenger->>'nombre'),nullif(btrim(coalesce(v_passenger->>'puesto','')),''),nullif(btrim(coalesce(v_passenger->>'contacto','')),''),nullif(btrim(coalesce(v_passenger->>'asiento','')),''));
        end if;
    end loop;
    update public.vehiculos set estado='asignado',proyecto=v_row.proyecto,asignado_a=v_driver_name,responsable=v_responsible_name,kilometraje=greatest(coalesce(kilometraje,0),v_row.kilometraje_salida),updated_at=now() where id=v_vehicle_id;
    return v_row;
end;
$$;
revoke all on function public.crm_guardar_salida_vehiculo_v148(jsonb) from public,anon;
grant execute on function public.crm_guardar_salida_vehiculo_v148(jsonb) to authenticated,service_role;

create or replace function public.crm_cerrar_salida_vehiculo_v148(p_viaje_id bigint,p_datos jsonb default '{}'::jsonb)
returns public.vehiculos_viajes
language plpgsql security definer set search_path=public
as $$
declare v_row public.vehiculos_viajes%rowtype;v_return timestamptz;v_km numeric;v_base jsonb;v_return_note text;
begin
    if auth.uid() is null or not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen']) then raise exception using errcode='42501',message='No tienes permiso para registrar el regreso.'; end if;
    if not public.crm_validar_checklist_vehiculo_v148(coalesce(p_datos->'checklist_entrada','{}'::jsonb)) then raise exception 'Completa correctamente los 28 puntos del checklist de entrada.'; end if;
    select * into v_row from public.vehiculos_viajes where id=p_viaje_id for update;
    if not found then raise exception 'No existe la salida.'; end if;
    if v_row.estado<>'en_curso' then raise exception 'La salida ya fue finalizada.'; end if;
    if nullif(btrim(coalesce(v_row.responsable_firma,'')),'') is null then raise exception 'Falta la firma del Responsable asignado antes de cerrar el regreso.'; end if;
    v_return:=coalesce(nullif(p_datos->>'fecha_regreso_real','')::timestamptz,now());
    if v_return<v_row.fecha_salida then raise exception 'La fecha de regreso no puede ser anterior a la salida.'; end if;
    v_km:=greatest(v_row.kilometraje_salida,coalesce(nullif(p_datos->>'kilometraje_regreso','')::numeric,v_row.kilometraje_salida));
    update public.vehiculos_viajes set fecha_regreso_real=v_return,kilometraje_regreso=v_km,combustible_regreso=nullif(p_datos->>'combustible_regreso','')::numeric,vida_llantas_regreso=nullif(p_datos->>'vida_llantas_regreso','')::numeric,checklist_entrada=coalesce(p_datos->'checklist_entrada','{}'::jsonb),danos_entrada=coalesce(p_datos->'danos_entrada',danos_salida),observaciones=concat_ws(' | ',nullif(btrim(coalesce(observaciones,'')),''),nullif(btrim(coalesce(p_datos->>'observaciones','')),'')),estado='finalizado',updated_at=now() where id=p_viaje_id returning * into v_row;
    if coalesce(nullif(p_datos->>'actualizar_estado_base','')::boolean,false) then
        select to_jsonb(b) into v_base from public.vehiculos_estado_base b where b.vehiculo_id=v_row.vehiculo_id;
        v_base:=coalesce(v_base,v_row.estado_base_snapshot,'{}'::jsonb);
        v_return_note:=nullif(btrim(coalesce(p_datos->>'observaciones','')),'');
        v_base:=v_base || jsonb_build_object(
            'combustible_nivel_referencia',coalesce(nullif(p_datos->>'combustible_regreso','')::numeric,nullif(v_base->>'combustible_nivel_referencia','')::numeric),
            'vida_llantas',coalesce(nullif(p_datos->>'vida_llantas_regreso','')::numeric,nullif(v_base->>'vida_llantas','')::numeric),
            'kilometraje_referencia',v_km,
            'danos',coalesce(p_datos->'danos_entrada',v_row.danos_salida,'[]'::jsonb),
            'observaciones',case when v_return_note is null then nullif(v_base->>'observaciones','') else concat_ws(E'\n',nullif(v_base->>'observaciones',''),'Regreso '||to_char(v_return,'DD/MM/YYYY HH24:MI')||': '||v_return_note) end
        );
        perform public.crm_guardar_estado_base_vehiculo_v148(v_row.vehiculo_id,v_base);
    end if;
    update public.vehiculos set estado='disponible',proyecto=null,asignado_a=null,responsable=null,kilometraje=v_km,updated_at=now() where id=v_row.vehiculo_id;
    return v_row;
end;
$$;
revoke all on function public.crm_cerrar_salida_vehiculo_v148(bigint,jsonb) from public,anon;
grant execute on function public.crm_cerrar_salida_vehiculo_v148(bigint,jsonb) to authenticated,service_role;



create or replace function public.crm_firmar_responsable_salida_v148(p_viaje_id bigint,p_firma text,p_nombre text default null,p_empresa text default null)
returns public.vehiculos_viajes
language plpgsql security definer set search_path=public
as $$
declare v_row public.vehiculos_viajes%rowtype;v_sig text:=nullif(btrim(coalesce(p_firma,'')),'');
begin
    if auth.uid() is null or not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen']) then raise exception using errcode='42501',message='No tienes permiso para completar esta firma.'; end if;
    if v_sig is null or v_sig !~ '^data:image/(png|jpeg|jpg|webp);base64,' or length(v_sig)<100 then raise exception 'La firma del Responsable asignado no es válida.'; end if;
    select * into v_row from public.vehiculos_viajes where id=p_viaje_id for update;
    if not found then raise exception 'No existe la salida.'; end if;
    if v_row.estado<>'en_curso' then raise exception 'La salida ya fue finalizada.'; end if;
    update public.vehiculos_viajes set responsable_nombre=coalesce(nullif(btrim(coalesce(p_nombre,'')),''),responsable_nombre),responsable_empresa=coalesce(nullif(btrim(coalesce(p_empresa,'')),''),responsable_empresa),responsable_firma=v_sig,responsable_firmado_at=now(),updated_at=now() where id=p_viaje_id returning * into v_row;
    return v_row;
end;
$$;
revoke all on function public.crm_firmar_responsable_salida_v148(bigint,text,text,text) from public,anon;
grant execute on function public.crm_firmar_responsable_salida_v148(bigint,text,text,text) to authenticated,service_role;


create table if not exists public.vehiculos_firma_tokens(
    id bigint generated by default as identity primary key,
    viaje_id bigint not null references public.vehiculos_viajes(id) on update cascade on delete cascade,
...
create index if not exists vehiculos_firma_tokens_viaje_v148_idx on public.vehiculos_firma_tokens(viaje_id,expires_at desc);
alter table public.vehiculos_firma_tokens enable row level security;
drop policy if exists vehiculos_firma_tokens_admin_v148 on public.vehiculos_firma_tokens;
create policy vehiculos_firma_tokens_admin_v148 on public.vehiculos_firma_tokens for select to authenticated using(public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen']));
grant select on public.vehiculos_firma_tokens to authenticated;
grant usage,select on sequence public.vehiculos_firma_tokens_id_seq to authenticated;

create or replace function public.crm_crear_enlace_firma_vehiculo_v148(p_viaje_id bigint)
returns jsonb
language plpgsql security definer set search_path=public
as $$
declare v_trip public.vehiculos_viajes%rowtype;v_token text;v_hash text;v_expires timestamptz:=now()+interval '72 hours';
begin
    if auth.uid() is null or not public.crm_usuario_tiene_rol(array['administrador','jefe_almacen','almacen']) then raise exception using errcode='42501',message='No tienes permiso para generar enlaces de firma.'; end if;
    select * into v_trip from public.vehiculos_viajes where id=p_viaje_id;
    if not found then raise exception 'No existe la salida.'; end if;
    if v_trip.estado<>'en_curso' then raise exception 'Solo se puede generar un enlace para una salida activa.'; end if;
    if nullif(btrim(coalesce(v_trip.responsable_firma,'')),'') is not null then raise exception 'El Responsable asignado ya firmó esta salida.'; end if;
    v_token:='vf_'||replace(gen_random_uuid()::text,'-','')||replace(gen_random_uuid()::text,'-','');
    v_hash:=encode(digest(v_token,'sha256'),'hex');
    update public.vehiculos_firma_tokens set used_at=coalesce(used_at,now()) where viaje_id=p_viaje_id and used_at is null;
    insert into public.vehiculos_firma_tokens(viaje_id,token_hash,expires_at,created_by,created_at) values(p_viaje_id,v_hash,v_expires,auth.uid(),now());
    return jsonb_build_object('ok',true,'token',v_token,'expires_at',v_expires,'viaje_id',p_viaje_id,'folio',coalesce(v_trip.folio,''));
end;
$$;
revoke all on function public.crm_crear_enlace_firma_vehiculo_v148(bigint) from public,anon;
grant execute on function public.crm_crear_enlace_firma_vehiculo_v148(bigint) to authenticated,service_role;

create or replace function public.crm_portal_firma_vehiculo_v148(p_token text)
returns jsonb
language plpgsql stable security definer set search_path=public
as $$
declare v_hash text:=encode(digest(coalesce(p_token,''),'sha256'),'hex');v_token public.vehiculos_firma_tokens%rowtype;v_trip public.vehiculos_viajes%rowtype;v_vehicle public.vehiculos%rowtype;
begin
    select * into v_token from public.vehiculos_firma_tokens where token_hash=v_hash order by id desc limit 1;
    if not found then return jsonb_build_object('ok',false,'estado','invalido','mensaje','El enlace de firma no es válido.'); end if;
    if v_token.used_at is not null then return jsonb_build_object('ok',false,'estado','usado','mensaje','Este enlace ya fue utilizado.'); end if;
    if v_token.expires_at<now() then return jsonb_build_object('ok',false,'estado','vencido','mensaje','Este enlace de firma venció. Solicita uno nuevo.'); end if;
    select * into v_trip from public.vehiculos_viajes where id=v_token.viaje_id;
    if not found or v_trip.estado<>'en_curso' then return jsonb_build_object('ok',false,'estado','cerrado','mensaje','La salida ya no está disponible para firma.'); end if;
    select * into v_vehicle from public.vehiculos where id=v_trip.vehiculo_id;
    return jsonb_build_object('ok',true,'estado','pendiente','expires_at',v_token.expires_at,'viaje_id',v_trip.id,'folio',coalesce(v_trip.folio,''),'responsable_nombre',coalesce(v_trip.responsable_nombre,''),'responsable_empresa',coalesce(v_trip.responsable_empresa,''),'vehiculo',jsonb_build_object('nombre',coalesce(v_vehicle.numero_economico,''),'apodo',coalesce(v_vehicle.apodo,''),'marca',coalesce(v_vehicle.marca,''),'modelo',coalesce(v_vehicle.modelo,''),'placas',coalesce(v_vehicle.placas,'')),'destino',coalesce(v_trip.destino,''),'ciudad_estado',coalesce(v_trip.ciudad_estado,''),'proyecto',coalesce(v_trip.proyecto,''),'fecha_salida',v_trip.fecha_salida);
end;
$$;
revoke all on function public.crm_portal_firma_vehiculo_v148(text) from public;
grant execute on function public.crm_portal_firma_vehiculo_v148(text) to anon,authenticated,service_role;

create or replace function public.crm_portal_firmar_vehiculo_v148(p_token text,p_nombre text,p_empresa text,p_firma text)
returns jsonb
language plpgsql security definer set search_path=public
as $$
declare v_hash text:=encode(digest(coalesce(p_token,''),'sha256'),'hex');v_token public.vehiculos_firma_tokens%rowtype;v_trip public.vehiculos_viajes%rowtype;v_sig text:=nullif(btrim(coalesce(p_firma,'')),'');v_name text:=nullif(btrim(coalesce(p_nombre,'')),'');
begin
    select * into v_token from public.vehiculos_firma_tokens where token_hash=v_hash order by id desc limit 1 for update;
    if not found or v_token.used_at is not null or v_token.expires_at<now() then raise exception 'El enlace de firma es inválido, ya fue utilizado o está vencido.'; end if;
    select * into v_trip from public.vehiculos_viajes where id=v_token.viaje_id for update;
    if not found or v_trip.estado<>'en_curso' then raise exception 'La salida ya no está disponible para firma.'; end if;
    if v_name is null then v_name:=nullif(btrim(coalesce(v_trip.responsable_nombre,'')),''); end if;
    if v_name is null then raise exception 'Captura el nombre completo del Responsable asignado.'; end if;
    if v_sig is null or v_sig !~ '^data:image/(png|jpeg|jpg|webp);base64,' or length(v_sig)<100 then raise exception 'La firma no es válida.'; end if;
    update public.vehiculos_viajes set responsable_nombre=v_name,responsable_empresa=coalesce(nullif(btrim(coalesce(p_empresa,'')),''),responsable_empresa),responsable_firma=v_sig,responsable_firmado_at=now(),updated_at=now() where id=v_trip.id;
    update public.vehiculos_firma_tokens set used_at=now() where id=v_token.id;
    return jsonb_build_object('ok',true,'viaje_id',v_trip.id,'folio',coalesce(v_trip.folio,''),'firmado_at',now());
end;
$$;
revoke all on function public.crm_portal_firmar_vehiculo_v148(text,text,text,text) from public;
grant execute on function public.crm_portal_firmar_vehiculo_v148(text,text,text,text) to anon,authenticated,service_role;

insert into public.crm_migraciones(version,aplicada_at)
values('CRM-V148-ESTADO-BASE-INSPECCION-VISUAL-VEHICULOS-2026-08-28',now())
on conflict(version) do update set aplicada_at=excluded.aplicada_at;
