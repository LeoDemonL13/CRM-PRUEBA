...
function printMinutes(){
    ...
    win.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${html(title)}</title><style>body{font-family:Arial,sans-serif;color:#111;padding:32px;line-height:1.45}h1{font-size:22px;margin:0 0 18px;border-bottom:2px solid #00416B;padding-bottom:10px}pre{white-space:pre-wrap;font:12px/1.6 Arial,sans-serif}.brand{font-size:10px;color:#64748b;margin-bottom:8px;text-transform:uppercase;letter-spacing:.12em}@media print{body{padding:10mm}}</style></head><body><div class="brand">Nexus Obsidian · SKILL Reuniones</div><h1>${html(title)}</h1><pre>${html(body.replace(title+'\n',''))}</pre></body></html>`);win.document.close();win.focus();setTimeout(()=>win.print(),250);
}
