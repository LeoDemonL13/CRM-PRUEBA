...
(function(){
    ...
    function renderSystemOrders(query=''){
        ...
        const filtered=systemOrderGroups.filter(group=>!search||(window.SkilledSearch?.matches?window.SkilledSearch.matches([group.ordenCompra,group.proveedor,group.almacenNombre,...group.items.flatMap(item=>[item.materialCodigo,item.descripcion])],search):[group.ordenCompra,group.proveedor,group.almacenNombre,...group.items.flatMap(item=>[item.materialCodigo,item.descripcion])].some(value=>key(value).includes(search))));
