...
const shapeKey=value=>{const raw=normalize(value?.tipo??value);if(['pickup','pick_up','camioneta_pickup','camioneta_de_carga','troca','troka'].includes(raw))return'pickup';if(['camioneta','suv','suv_camioneta','utilitaria','utilitario','wagon','crossover','todoterreno'].includes(raw))return'suv';if(['automovil','auto','coche','sedan','hatchback','compacto','coupe'].includes(raw))return'car';if(['van','minivan','panel','furgon','furgoneta','sprinter','urban'].includes(raw))return'van';if(['camion','tractocamion','tracto','camion_ligero','camion_mediano','rabon','torton'].includes(raw))return'truck';if(['motocicleta','moto'].includes(raw))return'motorcycle';if(['montacargas','forklift'].includes(raw))return'forklift';if(['generador_movil','generador','genny','planta_de_luz','planta_luz','planta_electrica'].includes(raw))return'equipment';if(['maquinaria_movil','maquinaria','retroexcavadora','minicargador','excavadora','bulldozer'].includes(raw))return'machinery';return'car'};
