...
function renderTripSeatMap(){
 ...
 const names=passengerNamesFromInput();
 ...
 const assignmentRows=names.map((name,index)=>{const selected=Object.entries(tripSeatAssignments).find(([,value])=>value===name)?.[0]||'';return`<label class="seat-assignment-row-v23"><div><strong>${esc(name)}</strong><span>Pasajero ${index+1}</span></div><select data-passenger-seat="${esc(name)}"><option value="">Sin asiento</option>${availableSeats.map(seat=>`<option value="${esc(seat.code)}" ${selected===seat.code?'selected':''}>${esc(seat.label)} · Fila ${seat.row}</option>`).join('')}</select></label>`}).join('');
 map.classList.add('trip-seat-map-v23');
 map.innerHTML=seatShellHtml(vehicle,rowsHtml,rows.length)+`<div class="seat-assignment-panel-v23"><h4>Asignación de pasajeros</h4><p>Selecciona el lugar desde esta lista. También puedes tocar un asiento libre para asignar automáticamente al siguiente pasajero pendiente.</p><div class="seat-assignment-grid-v23">${assignmentRows||'<div class="text-[9px] text-gray-500">Escribe primero los pasajeros para asignar sus lugares.</div>'}</div></div>`;
