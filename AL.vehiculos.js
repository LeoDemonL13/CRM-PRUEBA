...
function renderTripSeatMap(){
 ...
 const driver=clean($('trip-driver').value)||'Conductor pendiente';const names=passengerNames();
 ...
 const assignedNames=new Set(Object.values(tripSeatAssignments));const freeNames=names.filter(name=>!assignedNames.has(name));
 seats.filter(seat=>!seat.driver).forEach(seat=>{if(!tripSeatAssignments[seat.code]&&freeNames.length)tripSeatAssignments[seat.code]=freeNames.shift()});
 ...
 const rowsHtml=rows.map(row=>{const rowSeats=seats.filter(seat=>seat.row===row);return`<div class="seat-row ${seatRowClass(rowSeats.length)}" style="--seat-count:${rowSeats.length}"><span class="seat-row-label">Fila ${row}</span>${rowSeats.map(seat=>seat.driver?`<div class="seat is-driver is-assigned">${seatGraphicHtml(true)}<span class="seat-code">CONDUCTOR</span><span class="seat-name" title="${esc(driver)}">${esc(driver)}</span></div>`:`<div class="seat ${tripSeatAssignments[seat.code]?'is-assigned':''}">${seatGraphicHtml(false)}<span class="seat-code">${esc(seat.label)}</span><select data-seat-code="${esc(seat.code)}"><option value="">Sin asignar</option>${names.map(name=>`<option value="${esc(name)}" ${tripSeatAssignments[seat.code]===name?'selected':''}>${esc(name)}</option>`).join('')}</select></div>`).join('')}</div>`}).join('');
 map.innerHTML=seatShellHtml(vehicle,rowsHtml,rows.length);
 map.querySelectorAll('[data-seat-code]').forEach(select=>select.addEventListener('change',()=>{const code=select.dataset.seatCode;const previous=tripSeatAssignments[code];if(previous)delete tripSeatAssignments[code];const chosen=clean(select.value);if(chosen){Object.keys(tripSeatAssignments).forEach(key=>{if(tripSeatAssignments[key]===chosen)delete tripSeatAssignments[key]});tripSeatAssignments[code]=chosen}renderTripSeatMap()}));
...
function vehicleMatches(item){const query=searchValue();return !query||[item.numeroEconomico,item.placas,item.vin,item.marca,item.modelo,item.tipo,item.proyecto,item.asignadoA,item.responsable].some(value=>lower(value).includes(query))}
...
function tripMatches(item){const query=searchValue();return !query||[item.vehiculo?.numeroEconomico,item.vehiculo?.placas,item.conductor,item.proyecto,item.destino,item.motivo,...(item.pasajeros||[]).map(p=>p.nombre)].some(value=>lower(value).includes(query))}
function expenseMatches(item){const query=searchValue();return !query||[item.vehiculo?.numeroEconomico,item.vehiculo?.placas,item.tipo,item.proveedor,item.comprobante,item.notas].some(value=>lower(value).includes(query))}
...
async function removeVehicle(){if(!editingVehicle||!canDelete())return;if(!confirm(`¿Eliminar definitivamente el vehículo de prueba ${editingVehicle.numeroEconomico}?\n\nTambién se eliminarán sus viajes, pasajeros y gastos registrados. Usa esta opción solo para limpiar datos de prueba.`))return;const value=prompt(`Para confirmar, escribe su número económico:
...
function mapVehicleImportRow(raw,index){
 ...
 const numberEconomic=clean(get('No. económico','Numero economico','No economico')).toUpperCase();
 ...
 if(!numberEconomic)errors.push('Falta número económico');if(!brand)errors.push('Falta marca');if(!model)errors.push('Falta modelo');if(!rawType)errors.push('Falta tipo');else if(!type)errors.push(`Tipo no permitido: ${rawType}`);
 ...
 const duplicateFile=vehicleImportRows.some(item=>lower(item.payload.numeroEconomico)===lower(numberEconomic));if(numberEconomic&&duplicateFile)errors.push('Número económico repetido en el archivo');
