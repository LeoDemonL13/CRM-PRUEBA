...
const shapeKey=value=>{const raw=normalize(value?.tipo??value);if(['pickup','pick_up','camioneta_pickup','camioneta_de_carga'].includes(raw))return'pickup';if(['camioneta','suv','suv_camioneta','utilitaria','wagon'].includes(raw))return'suv';if(['automovil','auto','coche','sedan','hatchback','compacto'].includes(raw))return'car';if(['van','minivan','panel','furgon','furgoneta'].includes(raw))return'van';if(['camion','tractocamion','camion_ligero','camion_mediano'].includes(raw))return'truck';if(['motocicleta','moto'].includes(raw))return'motorcycle';if(['montacargas','forklift'].includes(raw))return'forklift';if(['generador_movil','generador','genny','planta_de_luz'].includes(raw))return'equipment';if(['maquinaria_movil','maquinaria','retroexcavadora','minicargador'].includes(raw))return'machinery';return'car'};
