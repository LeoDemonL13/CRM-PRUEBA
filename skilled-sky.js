(function () {
    ...
    const profileSourceAccess = {
        ...
        jefe_almacen:new Set(['materiales','categorias','almacenes','herramientas','herramientas_asignaciones','vehiculos','proyectos','compras']),
        almacen:new Set(['materiales','categorias','almacenes','herramientas','herramientas_asignaciones','vehiculos','proyectos','compras']),
        compras:new Set(['materiales','categorias','proyectos','proveedores','proveedor_materiales','compras','cotizaciones','solicitudes_proveedor','servicios']),
        proyectos:new Set(['proyectos','materiales','categorias','compras']),
        planeacion:new Set(['proyectos','materiales','categorias','compras']),
        coordinacion:new Set(['proyectos','vehiculos','compras']),
        logistica:new Set(['proyectos','vehiculos','materiales','categorias']),
        recepcion:new Set(['presencia_recepcion','directorio_recepcion','suministros','tienda']),
        rh:new Set(['personal','proyectos','vehiculos','asistencia','asignaciones_rh','incidencias_rh','documentos_rh','capacitaciones_rh','participantes_capacitacion_rh','activos_rh','resguardos_rh']),
        finanzas:new Set(['proyectos','compras','cotizaciones']),
        tsi:new Set(['materiales','proyectos','categorias']),
        sky_demo:new Set([]),
        consulta:new Set(['materiales','categorias'])
    };
    ...
    function currentRole() { return authenticatedRole() || 'consulta'; }
    function detectProfile() {
        const role = authenticatedRole();
        if (['administrador','jefe_almacen','almacen','compras','rh','finanzas','gerente_general','subgerente','sky_demo','proyectos','planeacion','coordinacion','logistica','recepcion','tsi','consulta'].includes(role)) return role;
        if (requestedProfile && skyProfiles.has(requestedProfile)) return requestedProfile;
    ...
    function profileConfig(profile = detectProfile()) {
        const base = {
            ...
            recepcion: { title:'Skill · Asistente de Recepción', subtitle:'Suministros, listas de Tienda, atención de visitantes, presencia por área y avisos internos. Solo consulta información autorizada para Recepción.', placeholder:'Ej. ¿Qué suministros están por acabarse?', examples:[['Suministros','¿Qué suministros están bajo mínimo?'],['Buscar','¿Tenemos papel higiénico?'],['Tienda','¿Qué listas de tienda siguen pendientes?'],['Presencia','¿Hay alguien de Compras aquí?'],['Puerta','Leobardo está en la entrada y pide que le abran'],['Aviso','Avisa a Compras que llegó un proveedor']] },
    ...
    function commandNormalize(value) {
        ...
        const replacements = [
            ...
            [/\bsubministro(s)?\b/g, 'suministro$1'], [/\bsuminstro(s)?\b/g, 'suministro$1'], [/\bsuminiztro(s)?\b/g, 'suministro$1'],
            [/\bcotizasion(es)?\b/g, 'cotizacion$1'], [/\bcotisacion(es)?\b/g, 'cotizacion$1'], [/\brequisision(es)?\b/g, 'requisicion$1'],
            [/\brec humanos\b/g, 'recursos humanos'], [/\bres humanos\b/g, 'recursos humanos'], [/\brec humano\b/g, 'recursos humanos'],
            [/\bechale ojo\b/g, 'revisa'], [/\bechale una mirada\b/g, 'revisa'], [/\bfijate en\b/g, 'revisa'],
            [/\bjalame\b/g, 'busca'], [/\bsacame\b/g, 'busca'], [/\bpasame\b/g, 'busca'],
...
@keyframes skyPulse{0%{box-shadow:0 0 0 0 rgba(96,165,250,.35)}70%{box-shadow:0 0 0 9px rgba(96,165,250,0)}100%{box-shadow:0 0 0 0 rgba(96,165,250,0)}}
    ...
    let meetingModulePromise = null;
    function loadMeetingModule() {
        if (window.SkilledMeetings) return Promise.resolve(window.SkilledMeetings);
        if (meetingModulePromise) return meetingModulePromise;
        meetingModulePromise = new Promise((resolve, reject) => {
            const existing = [...document.scripts].find(node => /skilled-meetings\.js/i.test(node.src || ''));
            ...
            if (!existing) { script.src = 'skilled-meetings.js?v=145'; script.async = true; document.head.appendChild(script); }
    ...
    async function openMeetingAssistant(options={}) {
        try {
            ...
            setStatus(error?.message || 'No se pudo abrir Modo reunión.', 'error');
            return false;
        }
    }
    ...
    function createUi() {
        ...
        if (!document.getElementById('sky-overlay')) {
            ...
            const scopeLabel=isExecutiveReadProfile(profile)?'Acceso ejecutivo · todas las áreas autorizadas':`Acceso limitado · ${profileNames[profile]||profile}`;
            modal.innerHTML = `
                <section class="sky-modal" role="dialog" aria-modal="true" aria-labelledby="sky-title">
                    <header class="sky-head">
                        <div class="flex items-center gap-3"><div class="sky-orb"></div><div class="min-w-0"><div id="sky-title" class="sky-title">${html(config.title)}</div><div class="sky-subtitle">${html(config.subtitle)} Los datos del CRM se consultan respetando los permisos de tu perfil.</div><div class="sky-scope-badge ${isExecutiveReadProfile(profile)?'exec':''}">${html(scopeLabel)}</div></div></div>
                        <div class="sky-head-actions"><button id="sky-meeting-mode" class="sky-head-tool wide" type="button" title="SKILL Reuniones · minuta y diferenciación de voces" aria-label="Abrir modo reunión"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M8 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm8 0a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/><path d="M2.5 20a5.5 5.5 0 0 1 11 0M10.5 20a5.5 5.5 0 0 1 11 0"/></svg>Reunión</button><button id="sky-clear" class="sky-head-tool" type="button" title="Nueva conversación" aria-label="Nueva conversación">↺</button><button id="sky-settings-button" class="sky-head-tool" type="button" title="Micrófono y voz" aria-label="Micrófono y voz">⚙</button><button id="sky-close" class="sky-close" type="button" aria-label="Cerrar">×</button></div>
                    </header>
                    <div class="sky-body">
            ...
            document.getElementById('sky-meeting-mode')?.addEventListener('click', openMeetingAssistant);
    ...
    function pageAwareExamples(config){
        ...
        extra.push(['Modo reunión','Abre modo reunión para tomar una minuta y diferenciar las voces']);
    ...
    function navigationOptions(profile=detectProfile()){
        ...
        const maps={
            ...
            compras:{inicio:'CO.inicio.html',proveedores:'CO.proveedores.html',cotizaciones:'CO.cotizaciones.html','hacer compra':'CO.hacer-compra.html',compras:'CO.hacer-compra.html',requisiciones:'CO.requisiciones.html',recepciones:'CO.recepciones.html',servicios:'CO.servicios.html','bajo minimo':'CO.bajo-minimo.html'},
    ...
    function workflowRoutes(profile = detectProfile()) {
        const maps = {
            almacen: [
                [/\b(registrar|hacer|capturar|meter)\b.*\b(entrada|salida|traspaso|prestamo|préstamo|movimiento)\b/i,'AL.movimientos.html','Registrar movimiento'],
                [/\b(escanear|scanner|lector|codigo de barras|código de barras|qr)\b/i,'AL.escaner.html','Abrir escáner'],
                [/\b(imprimir|generar|sacar)\b.*\b(etiqueta|etiquetas|qr|codigo|código)\b/i,'AL.etiquetas.html','Generar etiquetas'],
                [/\b(nuevo|agregar|alta|registrar)\b.*\b(material|producto)\b/i,'AL.catalogo.html','Abrir catálogo']
            ],
            jefe_almacen: [],
            compras: [
                [/\b(leer|analizar|cargar|subir|capturar)\b.*\b(cotizacion|cotización|pdf|respuesta.*proveedor)\b/i,'CO.cotizaciones.html','Analizar cotización'],
                [/\b(nuevo|agregar|registrar|alta)\b.*\b(proveedor)\b/i,'CO.proveedores.html','Abrir proveedores'],
                [/\b(hacer|preparar|procesar)\b.*\b(compra|orden de compra)\b/i,'CO.hacer-compra.html','Preparar compra'],
                [/\b(revisar|ver|consultar)\b.*\b(requisicion|requisición|requisiciones)\b/i,'CO.requisiciones.html','Abrir requisiciones']
            ],
            rh: [
                [/\b(nuevo|alta|agregar|registrar)\b.*\b(personal|trabajador|empleado|colaborador)\b/i,'RH.personal.html','Abrir Personal'],
                [/\b(revisar|ver|capturar)\b.*\b(asistencia|checador|incidencia)\b/i,'RH.asistencias.html','Abrir asistencias'],
                [/\b(calcular|revisar|generar)\b.*\b(nomina|nómina|pago)\b/i,'RH.nomina.html','Abrir nómina'],
                [/\b(asignar|registrar|ver)\b.*\b(equipo|computadora|resguardo)\b/i,'RH.equipos.html','Abrir equipos y resguardos']
            ],
            recepcion: [
            ...
            finanzas: [
                ...
                [/\b(revisar|ver)\b.*\b(cuenta.*pagar|pago|pagos)\b/i,'FI.cuentas-pagar.html','Abrir cuentas por pagar'],
                [/\b(revisar|crear|ver)\b.*\b(presupuesto)\b/i,'FI.presupuestos.html','Abrir presupuestos']
            ],
            planeacion: [
                [/\b(importar|cargar|subir)\b.*\b(proyecto|proyectos|excel|plantilla)\b/i,'PROY.importar.html?perfil=planeacion','Importar proyectos'],
                [/\b(revisar|ver|abrir)\b.*\b(solicitud|solicitudes|materiales solicitados)\b/i,'AL.solicitudes-material.html?perfil=planeacion','Abrir solicitudes'],
                [/\b(revisar|ver|abrir)\b.*\b(proyecto|proyectos|planeacion|planeación)\b/i,'AL.proyectos.html?perfil=planeacion','Abrir proyectos']
            ],
            coordinacion: [
                [/\b(revisar|ver|abrir)\b.*\b(solicitud|solicitudes|materiales)\b/i,'AL.solicitudes-material.html?perfil=coordinacion','Abrir solicitudes'],
                [/\b(revisar|ver|abrir)\b.*\b(vehiculo|vehículo|flotilla|transporte)\b/i,'AL.vehiculos.html?perfil=coordinacion','Abrir vehículos'],
                [/\b(revisar|ver|abrir)\b.*\b(proyecto|proyectos)\b/i,'AL.proyectos.html?perfil=coordinacion','Abrir proyectos']
            ],
            logistica: [
                [/\b(revisar|ver|abrir)\b.*\b(entrega|entregas|direccion de entrega|dirección de entrega)\b/i,'CO.entregas.html?perfil=logistica','Abrir entregas'],
                [/\b(revisar|ver|abrir)\b.*\b(vehiculo|vehículo|flotilla|transporte)\b/i,'AL.vehiculos.html?perfil=logistica','Abrir vehículos'],
                [/\b(revisar|ver|abrir)\b.*\b(proyecto|proyectos)\b/i,'AL.proyectos.html?perfil=logistica','Abrir proyectos']
            ],
            proyectos: [
                [/\b(revisar|ver|abrir)\b.*\b(solicitud|solicitudes|materiales solicitados)\b/i,'AL.solicitudes-material.html','Abrir solicitudes'],
                [/\b(revisar|ver|abrir)\b.*\b(proyecto|proyectos|alcance|plan)\b/i,'AL.proyectos.html','Abrir proyectos'],
                [/\b(revisar|ver|abrir)\b.*\b(movimiento|movimientos|historial)\b/i,'AL.historial-movimientos.html','Abrir movimientos']
            ],
            tsi: [[/\b(solicitar|pedir|necesito)\b.*\b(epp|casco|guante|lente|chaleco)\b/i,'TSI.solicitudes-epp.html','Solicitar EPP']],
            gerente_general: [[/\b(revisar|ver)\b.*\b(proyecto|proyectos)\b/i,'GG.proyectos.html','Abrir proyectos'],[/\b(revisar|ver)\b.*\b(vehiculo|vehículo|flotilla)\b/i,'GG.vehiculos.html','Abrir vehículos']],
            subgerente: [[/\b(revisar|ver)\b.*\b(proyecto|proyectos)\b/i,'SG.proyectos.html','Abrir proyectos'],[/\b(revisar|ver)\b.*\b(vehiculo|vehículo|flotilla)\b/i,'SG.vehiculos.html','Abrir vehículos']]
        };
        if (profile === 'jefe_almacen') maps.jefe_almacen = maps.almacen;
        return maps[profile] || [];
    }
    function tryWorkflowIntent(raw) {
        const value = text(raw);
        for (const [pattern, href, label] of workflowRoutes()) {
            if (!pattern.test(value)) continue;
            setAnswer('Siguiente paso',`${label}.`,'Puedo llevarte al apartado correcto sin saltarme permisos. La operación final queda bajo tu confirmación.',[],{href,label});
            setTimeout(()=>{location.href=href},360);
            return {handled:true,voice:`Abriendo ${label.toLowerCase()}.`};
        }
        return null;
    }

    async function answerInterfaceAction(raw){
        const norm=commandNormalize(raw);
        if(/\b(abre|abrir|muestra|mostrar)\b.*\b(chat|mensajes|chat interno)\b/.test(norm)){
            try{const chat=await ensureChatModule();await chat.open();return{handled:true,voice:'Abrí el Chat interno.'}}
            catch(error){setAnswer('Chat no disponible','No pude abrir el Chat interno en este momento.',error?.message||'Intenta nuevamente.');return{handled:true,voice:'No pude abrir el Chat interno.'}}
        }
        if(/\b(actualiza|actualizar|recarga|recargar|refresca|refrescar)\b.*\b(pagina|página|pantalla|datos|esto|aqui|aquí)?\b/.test(norm)){
            const button=document.querySelector('[data-skilled-refresh],#refresh');
            if(button&&button!==document.getElementById('sky-send'))button.click();else location.reload();
            setAnswer('Actualizando','Estoy actualizando la información de esta pantalla.','Se conserva tu sesión y los permisos del perfil activo.');
            return{handled:true,voice:'Actualizando esta pantalla.'};
        }
        if(/\b(modo claro|tema claro|ponlo claro|pon tema claro)\b/.test(norm)){
            if(window.SkilledTheme?.toggleLight)await window.SkilledTheme.toggleLight(true);
            setAnswer('Apariencia','Activé el modo claro para tu cuenta.');return{handled:true,voice:'Modo claro activado.'};
        }
        if(/\b(modo oscuro|tema oscuro|ponlo oscuro|pon tema oscuro)\b/.test(norm)){
            if(window.SkilledTheme?.toggleLight)await window.SkilledTheme.toggleLight(false);
            setAnswer('Apariencia','Activé el modo oscuro para tu cuenta.');return{handled:true,voice:'Modo oscuro activado.'};
        }
        if(/\b(imprime|imprimir|saca una copia|sacar una copia)\b.*\b(pagina|página|pantalla|vista)\b/.test(norm)){
            setAnswer('Imprimir pantalla','Abriré el diálogo de impresión de esta vista.','La impresión final queda bajo tu confirmación.');
            setTimeout(()=>window.print(),220);return{handled:true,voice:'Preparando la impresión de esta vista.'};
        }
        return null;
    }
    ...
    function recognitionCandidateScore(value) {
        ...
        const profileWords = {
            ...
            compras: ['cotizacion','cotizaciones','cotizar','oferta','ofertas','precio','precios','plazo','plazos','entrega','comparar proveedores','orden de compra','requisicion','proveedor','recepcion','servicio','comprar','pago','vencimiento','rfc'],
            ...
            recepcion: ['recepcion','suministro','suministros','bodeguita','rack','racks','cajon','cajones','nivel','niveles','catalogo','tienda','lista','listas de compra','compras generales','papeleria','limpieza','consumible','stock','existencia','minimo','proveedor','precio','porteria','caseta','entrada','puerta','porton','acceso','afuera','visitante','visita','presencia','presente','compras','recursos humanos','almacen','finanzas','planeacion','coordinacion','logistica','tsi','gerencia','subgerencia','avisa','avisale','llama','dile','hablale','marcale','repartidor','mensajero','paquete','paqueteria','mercado libre','amazon','dhl','fedex','estafeta','ups','comida','pedido','lonche','materiales','remision'],
    ...
    function skyBridge(source, filter = '') {
        if (!window.SkilledDB?.getSkillProfileDataV136 && !window.SkilledDB?.getSkyProfileData) return null;
        return window.SkilledDB.getSkillProfileDataV136 ? SkilledDB.getSkillProfileDataV136(source, filter) : SkilledDB.getSkyProfileData(source, filter);
    }
    ...
    async function loadData(key) {
        ...
        const sourceForKey={
            materials:'materiales',low:'materiales',purchases:'compras',tools:'herramientas',assignments:'herramientas_asignaciones',vehicles:'vehiculos',projects:'proyectos',projectDetails:'proyectos',categories:'categorias',
            coSuppliers:'proveedores',coProviderMaterials:'proveedor_materiales',coSupplierRequests:'solicitudes_proveedor',coServices:'servicios',coQuotations:'cotizaciones',coStore:'tienda',
            rhPeople:'personal',rhAssignments:'asignaciones_rh',rhIncidents:'incidencias_rh',rhAttendance:'asistencia',rhDocuments:'documentos_rh',rhTrainings:'capacitaciones_rh',rhParticipants:'participantes_capacitacion_rh',rhOfficeAssets:'activos_rh',rhOfficeAssignments:'resguardos_rh',
            rePresence:'presencia_recepcion',reDirectory:'directorio_recepcion',reSupplies:'suministros',
            executiveRHOfficeAssets:'activos_rh',executiveTools:'herramientas',executiveWarehouses:'almacenes',executiveAlerts:'proyectos'
        };
        ...
        const loaders = {
            ...
            assignments: () => bridge ? skyBridge('herramientas_asignaciones') : SkilledDB.listToolAssignments({}),
            ...
            coProviderMaterials: () => bridge ? skyBridge('proveedor_materiales') : SkilledDB.listProviderMaterials({ activeOnly: true }),
            coSupplierRequests: () => bridge ? skyBridge('solicitudes_proveedor') : SkilledDB.listSupplierRequests({}),
            coServices: () => bridge ? skyBridge('servicios') : SkilledDB.listServices(),
            coStore: () => bridge && SkilledDB.getSkyProfileData ? skyBridge('tienda') : SkilledDB.listStoreRequests(),
            reSupplies: () => bridge && SkilledDB.getSkyProfileData ? skyBridge('suministros') : (typeof SkilledDB.listReceptionSupplies==='function' ? SkilledDB.listReceptionSupplies() : []),
            ...
            rhAssignments: () => bridge ? skyBridge('asignaciones_rh') : tableRows('rh_proyecto_asignaciones'),
            rhIncidents: () => bridge ? skyBridge('incidencias_rh') : tableRows('rh_incidencias'),
            rhAttendance: () => bridge ? skyBridge('asistencia') : (typeof SkilledDB.getSkyAttendanceV81 === 'function' ? SkilledDB.getSkyAttendanceV81() : []),
            rhDocuments: () => bridge ? skyBridge('documentos_rh') : tableRows('rh_documentos', '*,personal:rh_personal(id,numero_empleado,nombre,apellidos,puesto)'),
            rhTrainings: () => bridge ? skyBridge('capacitaciones_rh') : tableRows('rh_capacitaciones'),
            rhParticipants: () => bridge ? skyBridge('participantes_capacitacion_rh') : tableRows('rh_capacitacion_participantes', '*,personal:rh_personal(id,nombre,apellidos)'),
            rhOfficeAssets: () => bridge ? skyBridge('activos_rh') : SkilledDB.listRHOfficeAssets({ includeInactive: true }),
            rhOfficeAssignments: () => bridge ? skyBridge('resguardos_rh') : SkilledDB.listRHOfficeAssignments({ includeClosed: true }),
            ...
            rePresence: () => bridge ? skyBridge('presencia_recepcion') : (typeof SkilledDB.getReceptionPresenceV100==='function' ? SkilledDB.getReceptionPresenceV100('') : []),
            reDirectory: () => bridge ? skyBridge('directorio_recepcion') : (typeof SkilledDB.getReceptionRecognitionDirectoryV102==='function' ? SkilledDB.getReceptionRecognitionDirectoryV102() : []),
            executiveSearch: () => []
        };
    ...
    async function answerPurchasing(raw) {
        ...
        const [quotes, orders, suppliers, services] = await Promise.all([loadData('coQuotations'), loadData('purchases'), loadData('coSuppliers'), loadData('coServices')]);
        ...
        setAnswer('Resumen de Compras', `Hay ${quoteOpen} cotizaciones por revisar, ${groups.size} órdenes por atender y ${due} servicios próximos.`, 'Puedes preguntarme por una cotización, material a cotizar, OC, proveedor, precio, plazo o servicio.', cards, { href: 'CO.inicio.html', label: 'Abrir Compras' });
        return `En Compras hay ${quoteOpen} cotizaciones por revisar, ${groups.size} órdenes por atender y ${due} servicios próximos.`;
    }
    ...
    async function answerRH(raw) {
        ...
        if (/nomina|nómina|recibo de pago|comprobante de pago|pago semanal|whatsapp.*nomina|nomina.*whatsapp/.test(norm)) {
            ...
            if (error) { setAnswer('Nómina', 'El módulo de nómina todavía no está disponible en la base de datos.', 'Ejecuta SQL_MAESTRO_CRM.sql para activarlo.', [], {href:'RH.nomina.html',label:'Abrir nómina'}); return 'El módulo de nómina necesita la actualización V24.'; }
    ...
    function ensureChatModule() {
        ...
        chatModulePromise = new Promise((resolve, reject) => {
            ...
            if (!existing) { script.src = 'skilled-chat.js?v=145'; script.async = true; document.head.appendChild(script); }
    ...
    function prewarmSkyProfileData() {
        ...
        const keys=isExecutiveReadProfile(profile)
            ? ['projects','vehicles','materials']
            : [...new Set(profileSmartSearchConfig(profile).map(item=>item.key))].slice(0,3);
        const run=async()=>{for(const key of keys){try{await loadData(key)}catch(_){}}};
    ...
    async function answerReceptionOperations(raw){
        const norm=commandNormalize(raw);
        if(/\b(bodeguita|rack|racks|cajon|cajón|cajones|nivel|niveles|acomodar|acomoda|ubicacion fisica|ubicación física)\b/.test(norm)){setAnswer('Bodeguita','Puedo abrir la organización física de Suministros.','Desde Bodeguita puedes crear racks y niveles manualmente y arrastrar suministros a cada cajón.',[],{href:'RE.bodeguita.html',label:'Abrir Bodeguita'});return 'Abro Bodeguita para que organices los suministros por rack, nivel y cajón.';}
        if(/\b(suministro|suministros|catalogo|catálogo|papeleria|papelería|limpieza|consumible|consumibles|stock|existencia|existencias|bajo.*min|minimo|mínimo|por acabarse|reponer|reposicion|reposición)\b/.test(norm)){
            ...
            if(tokens.length){
                selected=window.SkilledSearch?.rank
                    ? window.SkilledSearch.rank(selected,tokens.join(' '),item=>[item.descripcion,item.categoria,item.marca,item.ubicacion,item.unidad,...(item.modismos||[])])
                    : selected.filter(item=>matchesTokens([item.descripcion,item.categoria,item.marca,item.ubicacion,item.unidad,...(item.modismos||[])],tokens));
            }
            ...
            return selected.length?`${headline} ${selected.slice(0,3).map(item=>`${item.descripcion}: ${formatNumber(item.stock)} ${item.unidad||''}`).join('. ')}`:headline;
        }
        if(/\b(tienda|lista.*compra|listas.*compra|compras generales|sams|sam's|aurrera|supermercado|mandado)\b/.test(norm)){
            const rows=await loadData('coStore');
            const pending=rows.filter(item=>!['comprado','cancelado'].includes(normalize(item.estado)));
            const tokens=searchTokens(raw,['tienda','lista','listas','compra','compras','general','generales','pendiente','pendientes','sams','aurrera','supermercado','mandado']);
            const base=tokens.length?rows.filter(item=>matchesTokens([item.folio,item.negocio,item.producto,item.marcaEspecifica,item.presentacion,item.solicitadoPor,item.responsableCompra,item.notas],tokens)):pending;
            const byFolio=new Map();
            base.forEach(item=>{const key=text(item.folio)||`SIN-${item.id}`;if(!byFolio.has(key))byFolio.set(key,[]);byFolio.get(key).push(item);});
            const groups=[...byFolio.entries()].slice(0,8);
            const cards=groups.map(([folio,items])=>{const total=items.reduce((sum,item)=>sum+number(item.costoEstimado)*number(item.cantidad),0);const urgent=items.some(item=>normalize(item.prioridad)==='urgente');return {title:`${folio}${urgent?' · Urgente':''}`,detail:`${items.length} partida${items.length===1?'':'s'} · estimado ${currency(total)} · ${items[0]?.negocio||'negocio por definir'}`};});
            const pendingFolios=new Set(pending.map(item=>item.folio).filter(Boolean)).size;
            setAnswer('Listas de Tienda',groups.length?`Encontré ${groups.length} lista${groups.length===1?'':'s'} relacionadas con la consulta.`:`No encontré listas con ese criterio. Hay ${pendingFolios} lista${pendingFolios===1?'':'s'} pendiente${pendingFolios===1?'':'s'} en total.`,'Desde Tienda puedes crear la lista y generar su hoja imprimible para entregar a quien realizará la compra.',cards,{href:'RE.tienda.html',label:'Abrir Tienda'});
            return groups.length?`Encontré ${groups.length} listas de Tienda. Hay ${pendingFolios} pendientes en total.`:`No encontré listas con ese criterio. Hay ${pendingFolios} pendientes en total.`;
        }
        return null;
    }
    ...
    async function answerSimple(raw) {
        ...
        const meetingCaptureAction=await answerMeetingCaptureAction(raw);if(meetingCaptureAction)return meetingCaptureAction;
        ...
        const interfaceAction=await answerInterfaceAction(raw);if(interfaceAction)return interfaceAction;
        const workflow=tryWorkflowIntent(raw);if(workflow)return workflow;
    ...
    async function answerExecutivePurchasing(raw) {
        ...
        cache.at = Date.now();
        if (/firm|autoriza|validar.*orden|orden.*validar|orden.*pendiente.*firma/.test(norm) && typeof SkilledDB.listExecutivePendingPurchaseOrdersV136==='function') {
            const pending=await SkilledDB.listExecutivePendingPurchaseOrdersV136();
            const cards=pending.slice(0,10).map(item=>({title:item.ordenCompra||'Orden de compra',detail:`${item.proveedor||'Proveedor por definir'} · ${item.firmadasCount||0}/4 firmas · ${item.pendientesEjecutivas?.join(' / ')||'pendiente'}`}));
            setAnswer('Órdenes por validar',pending.length?`${pending.length} orden${pending.length===1?'':'es'} de compra requiere${pending.length===1?'':'n'} tu firma ejecutiva.`:'No tienes órdenes de compra pendientes de firma.', 'Solo Gerencia General y Subgerencia pueden consultar este concentrado. Skill no firma documentos sin aprobación explícita.',cards,{href:`${detectProfile()==='gerente_general'?'GG':'SG'}.validar-ordenes-compra.html`,label:'Validar órdenes de compra'});
            return pending.length?`Tienes ${pending.length} órdenes de compra pendientes de firma.`:'No tienes órdenes de compra pendientes de firma.';
        }
    ...
    async function answerExecutive(raw) {
        const norm=commandNormalize(raw);
        const prefix=detectProfile()==='gerente_general'?'GG':'SG';
        if(/\b(suministro|suministros|papeleria|papelería|limpieza|consumible|consumibles|tienda|lista.*compra|listas.*compra|compras generales|supermercado|mandado)\b/.test(norm)){
            const receptionData=await answerReceptionOperations(raw);
            if(receptionData)return receptionData;
        }
    ...
    async function buildRelevantCRMContext(raw, profile=detectProfile()) {
        ...
        try {
            ...
            } else if (profile==='almacen'||profile==='jefe_almacen') {
    ...
    function profileSmartSearchConfig(profile) {
        ...
        return configs[profile] || (profile==='jefe_almacen'?configs.almacen:[]) || [];
    }
    ...
    function hasStrongLocalIntent(raw, profile = detectProfile()) {
        ...
        if (profile === 'compras' && /cotiz|proveedor|orden.*compra|requisicion|recepcion|servicio|comprar|entrega|precio|plazo|rfc|contacto|correo|email|whatsapp|telefono|quien.*vende|quién.*vende|quien.*surte|quién.*surte/.test(norm)) return true;
        if (profile === 'recepcion' && /suministro|suministros|catalogo|papeleria|limpieza|consumible|stock|existencia|bajo.*min|minimo|tienda|lista.*compra|compras generales|sams|aurrera|costo|precio|timbre|porteria|visitante|visita|comida|pedido|lonche|repartidor|paqueteria|mensajeria|mensajero|paquete|dhl|fedex|estafeta|ups|camara|rostro|entrada|puerta|porton|acceso|afuera|abrir|abreme|me abres|me abre|pasar|entrar|presente|presencia|hay alguien|esta alguien|quien esta|quien anda|sabes si esta|llego|llama|hablale|marcale|avisa|avisale|dile|que venga|que baje|busco a|necesito a|ocupo a|mercado libre|amazon|materiales|proveedor|remision|entrega/.test(norm)) return true;
    ...
    async function dispatchSkyAIPlan(plan, raw) {
        ...
        if (plan.intent === 'meeting') {
            const capture = await answerMeetingCaptureAction(queryText);
            if (capture) return capture.voice || null;
            const result = await answerMeetingAction(queryText);
            return result?.voice || null;
        }
        if (profile === 'recepcion') {
            if (plan.intent === 'store') return answerReceptionOperations(queryText);
            if (['material_family','material_stock','material_location','low_stock'].includes(plan.intent)) return answerReceptionOperations(queryText);
            if (['reception','rh_people','help','unknown'].includes(plan.intent)) { const op=await answerReceptionOperations(queryText); if(op)return op; const r=await answerReceptionPresenceAndAccess(queryText); if(r)return r.voice||null; const d=await answerReceptionDoorbell(queryText); return d?.voice||null; }
            if (plan.intent !== 'chat_message') return null;
        }
        if (executive && /\b(suministro|suministros|papeleria|papelería|limpieza|consumible|consumibles|tienda|lista.*compra|listas.*compra|compras generales|supermercado|mandado)\b/.test(commandNormalize(raw))) {
            const receptionData = await answerReceptionOperations(raw);
            if (receptionData) return receptionData;
        }
    ...
    async function answerUniversalIntent(raw) {
        ...
        if (/\b(que puedes hacer|que sabes hacer|que haces|dime que haces|para que sirves|cual es tu funcion|que me puedes resolver|ayudame|ayuda|como me ayudas|en que ayudas|capacidades|opciones de skill|opciones de sky)\b/.test(norm)) {
            ...
            const chatText=profile==='recepcion'?'Puedo consultar Suministros y listas de Tienda de Recepción, enviar avisos internos sobre personas en la entrada o entregas, y abrir SKILL Reuniones para tomar minutas. No consulto inventario de Almacén, Compras, RH ni información de otros perfiles.':'También puedo enviar mensajes por el Chat interno, generar convocatorias y abrir SKILL Reuniones para transcribir una junta y diferenciar voces dentro de la sesión. Por ejemplo: “Dile a Compras que ya llegó el material”, “Genera una reunión general a las 4” o “Abre modo reunión para tomar minuta”.';const voiceText='Si activas Modo conversación en la configuración del micrófono, después de responder vuelvo a escucharte automáticamente, como una conversación continua.';
        ...
        if (profile==='recepcion' && /\b(suministro|suministros|catalogo|catálogo|papeleria|papelería|limpieza|consumible|consumibles|tienda|lista.*compra|compras generales|sams|aurrera)\b/.test(norm)) return answerReceptionOperations(raw);
    ...
    async function dispatchByProfile(raw) {
        ...
        if (profile === 'recepcion') {const op=await answerReceptionOperations(raw).catch(()=>null);if(op)return op;const r=await answerReceptionPresenceAndAccess(raw);return r?.voice||null;}
